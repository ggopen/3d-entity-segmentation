...
`),s=new Blob([n],{type:"text/plain"});this._downloadBlob(s,"entities_model.obj")}_downloadJSON(e,t){const i=JSON.stringify(e,null,2),n=new Blob([i],{type:"application/json"});this._downloadBlob(n,t)}_downloadBlob(e,t){const i=URL.createObjectURL(e),n=document.createElement("a");n.href=i,n.download=t,document.body.appendChild(n),n.click(),document.body.removeChild(n),URL.revokeObjectURL(i)}destroy(){this.clear()}}class ve{constructor(e){this.viewer=e,this._rectMode=!1,this._startPoint=null,this._endPoint=null,this._rectCanvas=null,this._rectCtx=null,this._onRectSelectCallback=null,this._handler=null,this._pickHandler=null,this._resizeHandler=null,this._keyHandler=null,this._initRectCanvas(),this._initHandlers()}_initRectCanvas(){const e=this.viewer.containerId,t=document.getElementById(e);this._rectCanvas=document.getElementById("rectCanvas"),this._rectCanvas||(this._rectCanvas=document.createElement("canvas"),this._rectCanvas.id="rectCanvas",this._rectCanvas.style.position="absolute",this._rectCanvas.style.top="0",this._rectCanvas.style.left="0",this._rectCanvas.style.cursor="crosshair",this._rectCanvas.style.display="none",this._rectCanvas.style.zIndex="10",this._rectCanvas.style.pointerEvents="auto",t?(t.style.position=t.style.position||"relative",t.appendChild(this._rectCanvas)):document.body.appendChild(this._rectCanvas)),this._rectCtx=this._rectCanvas.getContext("2d")}_initHandlers(){this._handler=new Cesium.ScreenSpaceEventHandler(this._rectCanvas),this._handler.setInputAction(e=>{this._rectMode&&(this._startPoint={x:e.endPosition.x,y:e.endPosition.y},this._endPoint=null,this._clearRect())},Cesium.ScreenSpaceEventType.LEFT_DOWN),this._handler.setInputAction(e=>{!this._rectMode||!this._startPoint||(this._endPoint={x:e.endPosition.x,y:e.endPosition.y},this._drawRect())},Cesium.ScreenSpaceEventType.MOUSE_MOVE),this._handler.setInputAction(()=>{if(!this._rectMode)return;if(!this._startPoint||!this._endPoint){this._cancelRect();return}const e=this._getNormalizedRect();this._clearRect(),this._startPoint=null,this._endPoint=null,this._onRectSelectCallback&&e&&this._onRectSelectCallback(e),this._rectMode=!1,this._rectCanvas.style.display="none"},Cesium.ScreenSpaceEventType.LEFT_UP),this._handler.setInputAction(()=>{this._rectMode&&this._cancelRect()},Cesium.ScreenSpaceEventType.RIGHT_DOWN),this._resizeHandler=()=>this._updateCanvasSize(),window.addEventListener("resize",this._resizeHandler),this._keyHandler=e=>{e.key==="Escape"&&this._rectMode&&this._cancelRect()},window.addEventListener("keydown",this._keyHandler)}_updateCanvasSize(){if(!this._rectCanvas)return;const e=this.viewer.containerId,t=document.getElementById(e),i=this.viewer.getCanvas();if(t){const n=t.getBoundingClientRect();this._rectCanvas.width=i.width,this._rectCanvas.height=i.height,this._rectCanvas.style.width=n.width+"px",this._rectCanvas.style.height=n.height+"px"}else this._rectCanvas.width=i.width,this._rectCanvas.height=i.height}setRectMode(e){this._rectMode=e,e?(this._updateCanvasSize(),this._rectCanvas.style.display="block",this._startPoint=null,this._endPoint=null,this._clearRect()):(this._rectCanvas.style.display="none",this._clearRect(),this._startPoint=null,this._endPoint=null)}onRectSelect(e){this._onRectSelectCallback=e}_drawRect(){if(!this._startPoint||!this._endPoint)return;const e=this._rectCtx;e.clearRect(0,0,this._rectCanvas.width,this._rectCanvas.height);const t=Math.min(this._startPoint.x,this._endPoint.x),i=Math.min(this._startPoint.y,this._endPoint.y),n=Math.abs(this._endPoint.x-this._startPoint.x),s=Math.abs(this._endPoint.y-this._startPoint.y);e.fillStyle="rgba(79, 195, 247, 0.2)",e.fillRect(t,i,n,s),e.strokeStyle="#4fc3f7",e.lineWidth=2,e.strokeRect(t,i,n,s)}_clearRect(){this._rectCtx&&this._rectCtx.clearRect(0,0,this._rectCanvas.width,this._rectCanvas.height)}_cancelRect(){this._clearRect(),this._startPoint=null,this._endPoint=null,this._rectMode=!1,this._rectCanvas.style.display="none"}_getNormalizedRect(){if(!this._startPoint||!this._endPoint)return null;const e=Math.min(this._startPoint.x,this._endPoint.x),t=Math.min(this._startPoint.y,this._endPoint.y),i=Math.abs(this._endPoint.x-this._startPoint.x),n=Math.abs(this._endPoint.y-this._startPoint.y);if(i<5||n<5)return null;const s=this.viewer.getCanvas();return{x:e/s.width,y:t/s.height,width:i/s.width,height:n/s.height,pixelRect:{x:e,y:t,width:i,height:n}}}getScreenRect(){return this._getNormalizedRect()}screenToWorld(e,t){const i=this.viewer.viewer,n=i.camera.getPickRay(new Cesium.Cartesian2(e,t));return n?i.scene.globe.pick(n,i.scene.globe.ellipsoid):null}onEntityClick(e){this.removeEntityClickHandler(),this._pickHandler=new Cesium.ScreenSpaceEventHandler(this.viewer.viewer.scene.canvas),this._pickHandler.setInputAction(t=>{if(this._rectMode)return;const i=this.viewer.viewer.scene.pick(t.position);i&&i.primitive&&e(i)},Cesium.ScreenSpaceEventType.LEFT_CLICK)}removeEntityClickHandler(){this._pickHandler&&(this._pickHandler.destroy(),this._pickHandler=null)}getCurrentViewBounds(){const t=this.viewer.viewer.camera.computeViewRectangle();return t?{west:Cesium.Math.toDegrees(t.west),east:Cesium.Math.toDegrees(t.east),south:Cesium.Math.toDegrees(t.south),north:Cesium.Math.toDegrees(t.north)}:null}destroy(){this.setRectMode(!1),this._handler&&(this._handler.destroy(),this._handler=null),this.removeEntityClickHandler(),this._resizeHandler&&(window.removeEventListener("resize",this._resizeHandler),this._resizeHandler=null),this._keyHandler&&(window.removeEventListener("keydown",this._keyHandler),this._keyHandler=null),this._rectCanvas&&this._rectCanvas.parentNode&&this._rectCanvas.parentNode.removeChild(this._rectCanvas),this._rectCanvas=null,this._rectCtx=null}}function _e(){return new Promise((c,e)=>{if(window.Cesium){c();return}window.CESIUM_BASE_URL="./cesium/";const t=document.createElement("script");t.src="./cesium/Cesium.js",t.onload=()=>{window.Cesium?c():e(new Error("Cesium failed to load"))},t.onerror=()=>e(new Error("Cesium script load failed")),document.head.appendChild(t)})}const u={viewer:null,tileLoader:null,multiViewRenderer:null,segmentEngine:null,backProjector:null,spatialCluster:null,entityVisualizer:null,interactionController:null,entities:[],extractMode:"viewport",config:{views:["top","angle","front"],connectThreshold:.5,minFaces:20,filterGround:!0,groundThreshold:.1}},C=c=>document.getElementById(c);function L(c){const e=C("loadingOverlay");e.style.display="block",C("loadingText").textContent=c||"处理中..."}function E(){C("loadingOverlay").style.display="none"}function y(c){C("statusText").textContent=c}async function xe(){L("加载 Cesium 引擎..."),y("加载 Cesium 引擎...");try{await _e(),console.log("Cesium loaded:",typeof Cesium)}catch(c){console.error("Cesium load failed:",c),E(),y("Cesium 加载失败: "+c.message);return}L("初始化三维引擎..."),y("初始化中...");try{u.viewer=new de("cesiumContainer"),u.tileLoader=new me(u.viewer),u.multiViewRenderer=new fe(u.viewer),u.segmentEngine=new Ce,u.backProjector=new pe(u.viewer),u.spatialCluster=new we,u.entityVisualizer=new ye(u.viewer),u.interactionController=new ve(u.viewer),u.viewer.onTileCountChange(c=>C("tileCount").textContent=c),u.viewer.onFPSUpdate(c=>C("fpsCounter").textContent=c.toFixed(0)),u.interactionController.onRectSelect(async c=>{L("框选区域提取中..."),y("框选提取中..."),await Q(c),E()}),u.interactionController.onEntityClick(c=>{const e=u.entityVisualizer._entityPrimitives.indexOf(c.primitive);if(e>=0){const t=Math.floor(e/2);$(t)}}),E(),y("就绪 - 请加载瓦片数据"),Me(),W("viewport")}catch(c){console.error("初始化失败:",c),E();const e=c.message||String(c);e.includes("WebGL")?(y("WebGL 不可用 - 请使用支持 WebGL 的浏览器（Chrome/Firefox/Edge 最新版）"),be(e)):y("初始化失败: "+e)}}function be(c){const e=C("cesiumContainer");e.innerHTML=`
  ...
  `,e.style.background="#1a1a2e"}function Me(){C("loadTilesBtn").addEventListener("click",Te),C("flyHomeBtn").addEventListener("click",()=>u.viewer.flyHome()),C("extractBtn").addEventListener("click",Se),C("clearBtn").addEventListener("click",ke),C("modeViewportBtn").addEventListener("click",()=>W("viewport")),C("modeRectBtn").addEventListener("click",()=>W("rect")),document.querySelectorAll(".view-badge").forEach(c=>{c.addEventListener("click",()=>{const e=c.dataset.view;u.config.views.includes(e)?(u.config.views=u.config.views.filter(t=>t!==e),c.classList.remove("active")):(u.config.views.push(e),c.classList.add("active"))})}),C("threshold").addEventListener("input",c=>{u.config.connectThreshold=parseFloat(c.target.value),C("thresholdVal").textContent=c.target.value}),C("minFaces").addEventListener("input",c=>{u.config.minFaces=parseInt(c.target.value),C("minFacesVal").textContent=c.target.value}),C("filterGround").addEventListener("change",c=>{u.config.filterGround=c.target.checked}),C("exportGeoJSON").addEventListener("click",()=>{u.entities.length>0&&u.entityVisualizer.exportGeoJSON(u.entities)}),C("exportJSON").addEventListener("click",()=>{u.entities.length>0&&u.entityVisualizer.exportSpatialJSON(u.entities)})}function W(c){u.extractMode=c,C("modeViewportBtn").classList.toggle("active",c==="viewport"),C("modeRectBtn").classList.toggle("active",c==="rect"),u.interactionController.setRectMode(c==="rect"),y(c==="rect"?"请在场景中框选区域...":'就绪 - 点击"开始提取"进行全视口提取')}async function Te(){const c=C("tilesetUrl").value.trim();if(c){L("加载瓦片数据..."),y("加载瓦片中...");try{await u.tileLoader.load(c),y("瓦片加载完成 - 可开始实体提取"),C("extractBtn").disabled=!1}catch(e){console.error("瓦片加载失败:",e),y("瓦片加载失败: "+e.message)}finally{E()}}}async function Se(){u.extractMode==="viewport"?(L("全视口实体提取中..."),y("全视口提取中..."),await Q(null),E()):(y("请在场景中框选区域..."),u.interactionController.setRectMode(!0))}async function Q(c){const e=performance.now();try{const i=u.config.views.map(a=>c?u.multiViewRenderer.getViewWithRect(a,c):u.multiViewRenderer.getView(a));y("多视角渲染采样...");const n=await u.multiViewRenderer.renderViews(i);y("AI模型分割推理...");const s=await u.segmentEngine.segmentMultiView(n);y("掩码融合与三维反投影...");const r=u.tileLoader.getVisibleTriangles(),o=u.backProjector.projectTo3D(n,s,r);y("空间连通聚类...");const h=u.spatialCluster.cluster(o,{connectThreshold:u.config.connectThreshold,minFaces:u.config.minFaces,filterGround:u.config.filterGround,groundThreshold:u.config.groundThreshold});y("实体可视化..."),u.entityVisualizer.visualize(h),u.interactionController.onEntityClick(a=>{const d=u.entityVisualizer._entityPrimitives.indexOf(a.primitive);if(d>=0){const m=Math.floor(d/2);$(m)}}),u.entities=h;const l=((performance.now()-e)/1e3).toFixed(2);Ee(h,l),C("entityCountStatus").textContent=h.length,y(`提取完成 - ${h.length} 个实体, 耗时 ${l}s`),C("resultPanel").classList.remove("hidden")}catch(t){console.error("提取失败:",t),y("提取失败: "+t.message)}}function $(c){c<0||c>=u.entities.length||(u.entityVisualizer.highlightEntity(c),u.viewer.flyToEntity(u.entities[c]),document.querySelectorAll(".entity-item").forEach(e=>{e.classList.toggle("selected",parseInt(e.dataset.entityId)===c)}))}function Ee(c,e){C("entityCount").textContent=c.length,C("processTime").textContent=e+"s";const t=C("entityList");t.innerHTML="",c.forEach((i,n)=>{const s=document.createElement("div");s.className="entity-item",s.dataset.entityId=n;const r=i.boundingBox.center,o=i.area.toFixed(1),h=i.faces.length;s.innerHTML=`
    ...
    `,s.addEventListener("click",()=>$(n)),t.appendChild(s)})}function ke(){u.entities=[],u.entityVisualizer.clear(),u.segmentEngine.clear(),C("resultPanel").classList.add("hidden"),C("entityCountStatus").textContent="0",y("已清除")}xe();
