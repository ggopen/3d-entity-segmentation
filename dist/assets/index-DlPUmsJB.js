...
`),s=new Blob([n],{type:"text/plain"});this._downloadBlob(s,"entities_model.obj")}_downloadJSON(e,t){const i=JSON.stringify(e,null,2),n=new Blob([i],{type:"application/json"});this._downloadBlob(n,t)}_downloadBlob(e,t){const i=URL.createObjectURL(e),n=document.createElement("a");n.href=i,n.download=t,document.body.appendChild(n),n.click(),document.body.removeChild(n),URL.revokeObjectURL(i)}destroy(){this.clear()}}class _e{constructor(e){this.viewer=e,this._rectMode=!1,this._startPoint=null,this._endPoint=null,this._rectCanvas=null,this._rectCtx=null,this._onRectSelectCallback=null,this._handler=null,this._pickHandler=null,this._resizeHandler=null,this._keyHandler=null,this._initRectCanvas(),this._initHandlers()}_initRectCanvas(){const e=this.viewer.containerId,t=document.getElementById(e);this._rectCanvas=document.getElementById("rectCanvas"),this._rectCanvas||(this._rectCanvas=document.createElement("canvas"),this._rectCanvas.id="rectCanvas",this._rectCanvas.style.position="absolute",this._rectCanvas.style.top="0",this._rectCanvas.style.left="0",this._rectCanvas.style.cursor="crosshair",this._rectCanvas.style.display="none",this._rectCanvas.style.zIndex="10",this._rectCanvas.style.pointerEvents="auto",t?(t.style.position=t.style.position||"relative",t.appendChild(this._rectCanvas)):document.body.appendChild(this._rectCanvas)),this._rectCtx=this._rectCanvas.getContext("2d")}_initHandlers(){this._handler=new Cesium.ScreenSpaceEventHandler(this._rectCanvas),this._handler.setInputAction(e=>{this._rectMode&&(this._startPoint={x:e.endPosition.x,y:e.endPosition.y},this._endPoint=null,this._clearRect())},Cesium.ScreenSpaceEventType.LEFT_DOWN),this._handler.setInputAction(e=>{!this._rectMode||!this._startPoint||(this._endPoint={x:e.endPosition.x,y:e.endPosition.y},this._drawRect())},Cesium.ScreenSpaceEventType.MOUSE_MOVE),this._handler.setInputAction(()=>{if(!this._rectMode)return;if(!this._startPoint||!this._endPoint){this._cancelRect();return}const e=this._getNormalizedRect();this._clearRect(),this._startPoint=null,this._endPoint=null,this._onRectSelectCallback&&e&&this._onRectSelectCallback(e),this._rectMode=!1,this._rectCanvas.style.display="none"},Cesium.ScreenSpaceEventType.LEFT_UP),this._handler.setInputAction(()=>{this._rectMode&&this._cancelRect()},Cesium.ScreenSpaceEventType.RIGHT_DOWN),this._resizeHandler=()=>this._updateCanvasSize(),window.addEventListener("resize",this._resizeHandler),this._keyHandler=e=>{e.key==="Escape"&&this._rectMode&&this._cancelRect()},window.addEventListener("keydown",this._keyHandler)}_updateCanvasSize(){if(!this._rectCanvas)return;const e=this.viewer.containerId,t=document.getElementById(e),i=this.viewer.getCanvas();if(t){const n=t.getBoundingClientRect();this._rectCanvas.width=i.width,this._rectCanvas.height=i.height,this._rectCanvas.style.width=n.width+"px",this._rectCanvas.style.height=n.height+"px"}else this._rectCanvas.width=i.width,this._rectCanvas.height=i.height}setRectMode(e){this._rectMode=e,e?(this._updateCanvasSize(),this._rectCanvas.style.display="block",this._startPoint=null,this._endPoint=null,this._clearRect()):(this._rectCanvas.style.display="none",this._clearRect(),this._startPoint=null,this._endPoint=null)}onRectSelect(e){this._onRectSelectCallback=e}_drawRect(){if(!this._startPoint||!this._endPoint)return;const e=this._rectCtx;e.clearRect(0,0,this._rectCanvas.width,this._rectCanvas.height);const t=Math.min(this._startPoint.x,this._endPoint.x),i=Math.min(this._startPoint.y,this._endPoint.y),n=Math.abs(this._endPoint.x-this._startPoint.x),s=Math.abs(this._endPoint.y-this._startPoint.y);e.fillStyle="rgba(79, 195, 247, 0.2)",e.fillRect(t,i,n,s),e.strokeStyle="#4fc3f7",e.lineWidth=2,e.strokeRect(t,i,n,s)}_clearRect(){this._rectCtx&&this._rectCtx.clearRect(0,0,this._rectCanvas.width,this._rectCanvas.height)}_cancelRect(){this._clearRect(),this._startPoint=null,this._endPoint=null,this._rectMode=!1,this._rectCanvas.style.display="none"}_getNormalizedRect(){if(!this._startPoint||!this._endPoint)return null;const e=Math.min(this._startPoint.x,this._endPoint.x),t=Math.min(this._startPoint.y,this._endPoint.y),i=Math.abs(this._endPoint.x-this._startPoint.x),n=Math.abs(this._endPoint.y-this._startPoint.y);if(i<5||n<5)return null;const s=this.viewer.getCanvas();return{x:e/s.width,y:t/s.height,width:i/s.width,height:n/s.height,pixelRect:{x:e,y:t,width:i,height:n}}}getScreenRect(){return this._getNormalizedRect()}screenToWorld(e,t){const i=this.viewer.viewer,n=i.camera.getPickRay(new Cesium.Cartesian2(e,t));return n?i.scene.globe.pick(n,i.scene.globe.ellipsoid):null}onEntityClick(e){this.removeEntityClickHandler(),this._pickHandler=new Cesium.ScreenSpaceEventHandler(this.viewer.viewer.scene.canvas),this._pickHandler.setInputAction(t=>{if(this._rectMode)return;const i=this.viewer.viewer.scene.pick(t.position);i&&i.primitive&&e(i)},Cesium.ScreenSpaceEventType.LEFT_CLICK)}removeEntityClickHandler(){this._pickHandler&&(this._pickHandler.destroy(),this._pickHandler=null)}getCurrentViewBounds(){const t=this.viewer.viewer.camera.computeViewRectangle();return t?{west:Cesium.Math.toDegrees(t.west),east:Cesium.Math.toDegrees(t.east),south:Cesium.Math.toDegrees(t.south),north:Cesium.Math.toDegrees(t.north)}:null}destroy(){this.setRectMode(!1),this._handler&&(this._handler.destroy(),this._handler=null),this.removeEntityClickHandler(),this._resizeHandler&&(window.removeEventListener("resize",this._resizeHandler),this._resizeHandler=null),this._keyHandler&&(window.removeEventListener("keydown",this._keyHandler),this._keyHandler=null),this._rectCanvas&&this._rectCanvas.parentNode&&this._rectCanvas.parentNode.removeChild(this._rectCanvas),this._rectCanvas=null,this._rectCtx=null}}const xe=200;function d(c,e="info"){const t=new Date().toLocaleTimeString("zh-CN",{hour12:!1}),i=document.getElementById("logContent");if(i){const s={info:"#4fc3f7",success:"#81c784",warn:"#ffb74d",error:"#ef5350"}[e]||"#ccc",r=document.createElement("div");for(r.style.cssText=`color:${s};font-size:11px;line-height:1.5;padding:1px 0;word-break:break-all;`,r.textContent=`[${t}] ${c}`,i.appendChild(r),i.scrollTop=i.scrollHeight;i.children.length>xe;)i.removeChild(i.firstChild)}console.log(`[${e.toUpperCase()}] ${c}`)}function be(){const c=document.getElementById("logContent");c&&(c.innerHTML="")}function Ee(){return new Promise((c,e)=>{if(d("开始加载 Cesium 引擎...","info"),window.Cesium){d("Cesium 已存在，跳过加载","success"),c();return}window.CESIUM_BASE_URL="./cesium/",d(`CESIUM_BASE_URL 设置为: ${window.CESIUM_BASE_URL}`,"info");const t=document.createElement("script");t.src="./cesium/Cesium.js",t.async=!1;let i=!1;const n=setInterval(()=>{window.Cesium&&(clearInterval(n),i||(i=!0,d(`Cesium 加载完成 (版本 ${window.Cesium.VERSION||window.CESIUM_VERSION||"unknown"})`,"success"),c()))},100);t.onload=async()=>{if(d("Cesium.js 脚本加载完成，检查全局对象...","info"),await new Promise(s=>setTimeout(s,100)),window.Cesium){clearInterval(n),i||(i=!0,d(`Cesium 初始化成功 (版本 ${window.Cesium.VERSION||window.CESIUM_VERSION||"unknown"})`,"success"),c());return}if(d("Cesium.js 未自动设置 window.Cesium，使用包装加载...","warn"),clearInterval(n),!i)try{const s=await fetch("./cesium/Cesium.js");if(!s.ok)throw new Error("HTTP "+s.status);const o=`
  ...
  `,e.style.background="#1a1a2e",d("已显示 WebGL 不可用提示","warn")}function Te(){C("loadTilesBtn").addEventListener("click",ke),C("flyHomeBtn").addEventListener("click",()=>{u.viewer&&u.viewer.flyHome()}),C("extractBtn").addEventListener("click",Le),C("clearBtn").addEventListener("click",Ie),C("modeViewportBtn").addEventListener("click",()=>W("viewport")),C("modeRectBtn").addEventListener("click",()=>W("rect")),document.querySelectorAll(".view-badge").forEach(c=>{c.addEventListener("click",()=>{const e=c.dataset.view;u.config.views.includes(e)?(u.config.views=u.config.views.filter(t=>t!==e),c.classList.remove("active")):(u.config.views.push(e),c.classList.add("active"))})}),C("threshold").addEventListener("input",c=>{u.config.connectThreshold=parseFloat(c.target.value),C("thresholdVal").textContent=c.target.value}),C("minFaces").addEventListener("input",c=>{u.config.minFaces=parseInt(c.target.value),C("minFacesVal").textContent=c.target.value}),C("filterGround").addEventListener("change",c=>{u.config.filterGround=c.target.checked}),C("exportGeoJSON").addEventListener("click",()=>{u.entities.length>0&&(u.entityVisualizer.exportGeoJSON(u.entities),d("导出 GeoJSON 包围盒","success"))}),C("exportJSON").addEventListener("click",()=>{u.entities.length>0&&(u.entityVisualizer.exportSpatialJSON(u.entities),d("导出实体参数 JSON","success"))}),C("clearLogBtn").addEventListener("click",()=>{be(),d("日志已清空","info")}),C("toggleLogBtn").addEventListener("click",()=>{const c=C("logPanel");c.classList.toggle("collapsed");const e=C("toggleLogBtn");c.classList.contains("collapsed")?e.textContent="▲":e.textContent="▼"}),C("logPanelHeader").addEventListener("click",c=>{if(c.target.id==="clearLogBtn"||c.target.id==="toggleLogBtn")return;const e=C("logPanel");e.classList.toggle("collapsed");const t=C("toggleLogBtn");e.classList.contains("collapsed")?t.textContent="▲":t.textContent="▼"}),d("事件绑定完成","info")}function W(c){u.extractMode=c,C("modeViewportBtn").classList.toggle("active",c==="viewport"),C("modeRectBtn").classList.toggle("active",c==="rect"),u.interactionController.setRectMode(c==="rect"),c==="rect"?(v("请在场景中框选区域..."),d("切换到框选提取模式","info")):(v('就绪 - 点击"开始提取"进行全视口提取'),d("切换到全视口提取模式","info"))}async function ke(){const c=C("tilesetUrl").value.trim();if(!c){d("瓦片 URL 为空","warn");return}I("加载瓦片数据..."),v("加载瓦片中..."),d(`开始加载瓦片: ${c}`,"info");try{const e=performance.now();await u.tileLoader.load(c);const t=((performance.now()-e)/1e3).toFixed(2);v("瓦片加载完成 - 可开始实体提取"),C("extractBtn").disabled=!1,d(`瓦片加载完成，耗时 ${t}s`,"success")}catch(e){d("瓦片加载失败: "+e.message,"error"),v("瓦片加载失败: "+e.message)}finally{k()}}async function Le(){u.extractMode==="viewport"?(I("全视口实体提取中..."),v("全视口提取中..."),d("开始全视口实体提取","info"),await ee(null),k()):(v("请在场景中框选区域..."),d("等待用户框选区域...","info"),u.interactionController.setRectMode(!0))}async function ee(c){const e=performance.now();try{const t=u.config.views;d(`配置视角: ${t.join(", ")}`,"info"),v("多视角渲染采样..."),d("开始多视角渲染采样...","info");const i=t.map(a=>c?u.multiViewRenderer.getViewWithRect(a,c):u.multiViewRenderer.getView(a)),n=await u.multiViewRenderer.renderViews(i);d(`多视角渲染完成 (${n.length} 个视角)`,"success"),v("AI模型分割推理..."),d("开始分割推理...","info");const s=await u.segmentEngine.segmentMultiView(n);d("分割推理完成","success"),v("掩码融合与三维反投影..."),d("执行三维反投影...","info");const r=u.tileLoader.getVisibleTriangles();d(`可见三角面片数: ${r.length}`,"info");const o=u.backProjector.projectTo3D(n,s,r);d(`反投影面片数: ${o.length}`,"success"),v("空间连通聚类..."),d("执行空间连通聚类...","info");const h=u.spatialCluster.cluster(o,{connectThreshold:u.config.connectThreshold,minFaces:u.config.minFaces,filterGround:u.config.filterGround,groundThreshold:u.config.groundThreshold});d(`聚类完成: ${h.length} 个实体`,"success"),v("实体可视化..."),d("渲染实体可视化...","info"),u.entityVisualizer.visualize(h),u.interactionController.onEntityClick(a=>{const m=u.entityVisualizer._entityPrimitives.indexOf(a.primitive);if(m>=0){const g=Math.floor(m/2);J(g)}}),u.entities=h;const l=((performance.now()-e)/1e3).toFixed(2);Pe(h,l),C("entityCountStatus").textContent=h.length,v(`提取完成 - ${h.length} 个实体, 耗时 ${l}s`),C("resultPanel").classList.remove("hidden"),d(`实体提取完成: ${h.length} 个实体，耗时 ${l}s`,"success")}catch(t){d("提取失败: "+t.message,"error"),v("提取失败: "+t.message)}}function J(c){c<0||c>=u.entities.length||(u.entityVisualizer.highlightEntity(c),u.viewer.flyToEntity(u.entities[c]),d(`高亮实体 #${c+1}`,"info"),document.querySelectorAll(".entity-item").forEach(e=>{e.classList.toggle("selected",parseInt(e.dataset.entityId)===c)}))}function Pe(c,e){C("entityCount").textContent=c.length,C("processTime").textContent=e+"s";const t=C("entityList");t.innerHTML="",c.forEach((i,n)=>{const s=document.createElement("div");s.className="entity-item",s.dataset.entityId=n;const r=i.boundingBox.center,o=i.area.toFixed(1),h=i.faces.length;s.innerHTML=`
    ...
    `,s.addEventListener("click",()=>J(n)),t.appendChild(s)})}function Ie(){u.entities=[],u.entityVisualizer&&u.entityVisualizer.clear(),u.segmentEngine&&u.segmentEngine.clear(),C("resultPanel").classList.add("hidden"),C("entityCountStatus").textContent="0",v("已清除"),d("结果已清除","info")}Se();
